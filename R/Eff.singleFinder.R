#' Eff.singleFinder, to detect the normalized efficiencies of single reference genes (RGs)
#'
#' @family
...
#' @param num_group The number of experimental groups in rawd ata
#' @param num_sample The number of biological samples in your raw data matrix, it is equal to Num_rep multiplied by num_group
...
#' @param gsg.Ratio The ratio of the gold standard group filtered from all NFs, it is generally set to 0.01
...
#' @param Primer.E The mplification efficiency (E) of all primers, it shoulc be a vector here.
                             ...
                             Primer.E)
{
  ...
  Eff.ne <- function(data.pv, num_RG, name_RG, pv.th)
  {
    ...
    message = paste(
      ...
      "\n",
      "Jipan Zhang, Yongju Zhao: Normalized efficiency changes in reference genes (RG): a call to use multi-RG combination in qPCR experiments, 2021, Clinical Chemistry",
      "\n"
    )
    cat(message)
  }
